...
func killed(t *testing.T, h *host) *Daemon {
	...
	d := New(slog.New(slog.NewTextHandler(h.lines, nil)), events, h.Daemon.root, nil, nil)
...
func TestASecondSweepOverTheSameLogWritesNothing(t *testing.T) {
	...
	second := New(slog.New(slog.NewTextHandler(h.lines, nil)), first.events, first.root, nil, nil)
