...
- The Daemon gives OpenCode a per-Session config through `OPENCODE_CONFIG_CONTENT`, rather than editing a file the user owns. Two Sessions on different Models cannot fight over one file, and the Model chosen at Session start has somewhere to land. OpenCode documents this launch variable, and `session/new` still confirms the selected Model before the Session becomes Ready.
