...
- **Shape it against Pi and degrade for Hermes, not the reverse.** Pi gives a complete tool lifecycle — start, streaming update, end — on one stable `toolCallId`, plus per-turn `usage` and the provider and model by name. Hermes gives a start event and, for `read` and `edit` tools, **no completion at all** (12/12 consistent, on Linux as well as Windows). A Client that renders a tool call as "running" and waits will hang forever on every file operation. So the Event model must either synthesise a terminal state by tool kind or treat those kinds as fire-and-forget — decide which, and make it a rule rather than a patch.
