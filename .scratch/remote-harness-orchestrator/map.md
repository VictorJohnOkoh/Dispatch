...
**Prior art**: [T3 Code](../../docs/research/t3-code-prior-art.md) — the closest shipped analogue. Solves the Harness half (adapter per agent CLI, event-sourced core, headless server, loopback + tunnel reach) and deliberately not the Vendor half. Read before tickets 05, 06 and 10.

## Decisions so far

<!-- one line per resolved ticket -->

- [Hermes and Pi control surfaces](./issues/01-hermes-and-pi-control-surfaces.md) — **revised 2026-08-14 against a live install.** The two Harnesses have the **same integration shape**: both are subprocess-with-structured-stdio (`hermes acp` = JSON-RPC 2.0 over ndjson; `pi --mode rpc` = LF-delimited JSONL). The HTTP+SSE surface the original answer chose **does not exist** in Hermes v0.19.0. ACP also puts approval modes, session fork/list/resume and model selection **in-protocol**, so they are not orchestrator inventions. Approval remains non-uniform: Hermes gates in-protocol, Pi has nothing built in.
  - Superseded: *"incompatible integration shapes … Hermes best driven as a local HTTP+SSE server"*. Kept visible because the lesson generalises — this vendor's docs describe endpoints it does not ship, so `[docs]`-only claims need empirical confirmation before anything is designed against them.
  - **Approval interception confirmed 2026-08-15**, and it constrains the Event model in two ways. `session/request_permission` carries the **full diff before the write**, which is the richest control-plane payload on either Harness — but its `toolCallId` does not match the streamed `tool_call` id for the same edit, so approvals correlate on path and ordering, not id. And Hermes can answer *"denied by ACP client"* without ever asking the client: **a denial is not evidence of a decision**, so Approval Policy must distinguish *refused by a human* from *refused by default*. Bears on tickets 06 and 07.
  - **Approval settled on both Harnesses 2026-08-16: the Approval Policy must record its own decisions and treat Harness output as corroboration, not as the source of truth.** Neither Harness reports a refusal structurally — Hermes cannot distinguish a human's refusal from an internal failure, and Pi reports refusal only as a tool error with free text chosen by the extension author. Hermes also fails in **both** directions: its shell hook fails open, its ACP edit path fails closed. Pi's gate does work over RPC, but its approval request carries no tool-call id, so correlation is by ordering. Directly constrains ticket 07.
  - **Pi captured 2026-08-15, and it is the richer surface — shape the Event model against Pi and degrade for Hermes**, not the reverse. Pi gives a complete tool lifecycle (start / streaming update / end) with one stable `toolCallId`; Hermes gives a start event, no incremental output, an unreachable completion and an approval id that does not match its own tool-call id. Pi also carries per-turn `usage` and names the provider and model in the Control Plane stream, so cost accounting and Data-Plane correlation come free. Pointing Pi at a local Vendor is a `models.json` write, not an extension — simplifies ticket 02's Vendor story.
  - **Both Harnesses trap on stdin, in opposite directions**, so the Daemon must own stdin explicitly rather than inherit it. Pi's `-p` reads stdin to EOF even with the prompt as an argument (hangs forever at a terminal); `--mode rpc` is a session, not a pipe, and closing stdin truncates the run mid-turn. Hermes hangs because a spawned *child* inherited the pipe.
  - **Both Hermes defects proved Windows-specific on 2026-08-16** (WSL2, identical build): the deadlock went 118.83s → **1.73s**, and the phantom denial did not occur in 3/3 runs of the condition that triggered it 3/3 on Windows. **But one gap survives every platform: Hermes emits a tool completion only for `execute` tools, never for `read` or `edit`** (12/12 consistent). A Client that waits for a completion will hang forever on every file operation, so the Event model must synthesise a terminal state by tool kind. Bears on the Event model and the Client ticket.
  - Hermes also deadlocks on the first tool call of **any** kind on Windows (eight runs, always within ~6s of the client's own timeout) — one shared execution environment whose child inherits the ACP stdin pipe. **A Harness that appears hung may be waiting on its supervisor**, so the Daemon's supervision timeout is not a neutral observer: raising it raises the hang by the same amount. Worth remembering when the admission-control and health seams get drawn.
